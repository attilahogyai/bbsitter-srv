...
import org.mfr.xprt.rest.exception.MailingException;

public class MailHelper {
	static Pattern imageFinder = Pattern.compile("^.*?<img.*?src=\"(.*?)\"");
	static String mailPicDir=System.getProperty("mailsender.pic.dir","mailsender-pic");
	
	public static final List<String> DEFAULT_ADMIN_EMAIL = new ArrayList<String>();
	static{
		DEFAULT_ADMIN_EMAIL.add("postmaster@bbsitter.hu");
	}
	
	public static final String[] DEFAULT_SENDER = new String[]{"info@bbsitter.hu",""};
	...
	private static StringBuffer parseImages(String page,Map<String, String> fileMap) {
	...
	public static void sendHtmlMail(String sender[], List<String> recipients, String subject, String message, Map arguments) throws AddressException, UnsupportedEncodingException, MessagingException {
		...
		StringBuffer result = parseImages(message,fileMap);
		sendTextMail(sender, recipients, subject, result.toString(), arguments, false, null,  downloadImages(mailPicDir+"/"+org.mfr.rest.util.HttpHelper.replacetoAsciiChars(subject)+"/",fileMap));
	}
        ...
        if(! singlePart) {
			...
			if(attachmentFiles!=null && !attachmentFiles.isEmpty()){
				for(File file :attachmentFiles){ 
					...
					BodyPart messageBodyPartt = new MimeBodyPart();
					messageBodyPartt.setDataHandler(new DataHandler(fds));
					messageBodyPartt.setHeader("Content-ID", "<" + org.mfr.rest.util.HttpHelper.replacetoAsciiChars(file.getName()) + ">");
					multipart.addBodyPart(messageBodyPartt);
	...
	public static void sendErreReportMail(Exception e,String workflowid) {
		...
		address.add("postmaster@bbsitter.hu");
		...
		try {
			sendMail(address, "email/exceptionreport.vm", new String[]{"info@bbsitter.hu",""}, "error report ["+workflowid+"]", true, "text/plain",args);
		} catch (Exception e1) {
